...
// Reservar saca de la venta igual que vender: el titular baja solo, sin que
// nadie escriba un número.
test('reservar un lote lo saca del titular igual que venderlo', () => {
  const res = { ...inv, lotes: inv.lotes.map(l => l.n === 12 ? { ...l, estado: 'reservado' } : l) };
  assert.equal(construirVistaVitrina(res).titular, 'Los 6 que quedan');
});
...
test('los colocados no aparecen entre los disponibles', () => {
...
test('si todos los disponibles tienen 360, la pista lo promete sin condiciones', () => {
  const v = construirVistaVitrina(conPanos([6, 7, 8, 9, 11, 12, 13]));
